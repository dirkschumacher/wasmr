...
This is a super early version with probably bugs, missing features and not the best code quality. But it already works and I am happy to hear feedback.
...
The value returned from `get_memory_view` is a lazy raw vector that just stores a pointer to the `wasmer` memory. If you use the accessor methods `[i]` or request a continuous region `[1:n]` then only the necessary values from the linear memory will be returned as a `raw vector`.
...
* `I32/I64` are mapped to `IntegerVector` and `F32/F64` to `NumericVector`. Currently no way differentiate.
* WIP
...
* @romainfrancois 's [altrepisode](https://github.com/romainfrancois/altrepisode) and @jimhester 's [vroom](https://github.com/r-lib/vroom) on how to use ALTREP and C++.
